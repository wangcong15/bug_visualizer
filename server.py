...
class SimpleHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def deal_post_data(self):
        ...
        while remainbytes > 0:
            ...
            if boundary in line:
                ...
                out.write(preline)
                out.close()
                if out.name.endswith(".xml.zip"):
                    os.system("cd bug_history/;unzip " + out.name + ";rm " + out.name)
                    self.divide(out.name[0:-4] + "/")
    ...
    def divide(self, path):
        ...
        result = []
        pl0 = '<weakness class=\"(.*?)\">'
        pl = '<result weakness=\"(.*?)\" category=\"(.*?)\" desc=\"([\s\S]*?)\" severity=\"(.*?)\" confidence=\"(.*?)\">([\s\S]*?)</result>'
        pl2 = '<hint>(.*?)</hint>.*\n.*<position file=\"(.*?)\"[\s\S]*?line=\"(.*?)\"[\s\S]*?</position>'
        plCweId = '<integer>(.*?)</integer>'
        pattern0 = re.compile(pl0)
        pattern = re.compile(pl)
        pattern2 = re.compile(pl2)
        pattern3 = re.compile(plCweId)
        for file in files:
            if file.endswith(".xml"):
                ...
                try:
                    ...
                    class_name = pattern0.findall(all_the_text)
                    result_array = pattern.findall(all_the_text)
                    for result_a in result_array:
                        trace = result_a[5]
                        trace_array = pattern2.findall(trace)
                        cweids = pattern3.findall(trace)
                        result.append({'class':class_name[0], 'cwe':'', 'weakness':result_a[0], 'category':result_a[1], 'desc':result_a[2], 'severity':result_a[3], 'confidence':result_a[4], 'traces':[trace[1:]+trace[:1] for trace in trace_array], 'cweid':cweids})
                finally:
                    file_object.close( )
        json.dump(result, open(path + 'data.json', 'w'))
    ...
    def list_directory(self, path):
        ...
        f.write("</table>\n<hr>\n*文件只能在/bug_history/目录中上传，目前只支持zip格式和xml格式，同一组上传请对应相同的文件名。\n</body>\n</html>\n")
        length = f.tell()
        f.seek(0)
        self.send_response(200)
        self.send_header("Content-type", "text/html")
